  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 h-32 bg-[#E6D8AD]/40 pointer-events-none overflow-hidden">
        <svg
          className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full h-full opacity-15 text-[#D4AF37]"
          viewBox="0 0 400 200"
          preserveAspectRatio="none"
        >
          <path
            d="M0,200 L0,100 C0,44.77 89.54,0 200,0 C310.46,0 400,44.77 400,100 L400,200 Z"
            fill="currentColor"
          />
        </svg>
      </div>
      ...
      <div className="relative z-10 p-8 flex flex-col h-full">
        ...
        >
          ...
          <ChevronRight
            size={16}
            className={`${isRtl ? "rotate-180" : ""} transition-transform group-hover:translate-x-1 ${isRtl ? "group-hover:-translate-x-1" : ""}`}
          />
        </button>
      </div>
      ...
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none group-hover:opacity-[0.04] transition-opacity duration-300">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          ...
          <rect
            width="100%"
            height="100%"
            fill={`url(#hex-pattern-${trackNumber})`}
          />
        </svg>
      </div>
    </motion.div>
