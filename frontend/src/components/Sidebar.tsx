      ...
      >
        ...
        <div className="h-24 flex items-center px-6 border-b border-accent/10">
          <Link to="/" className="flex items-center gap-3">
            <div className="w-10 h-10 bg-accent rounded-lg flex items-center justify-center flex-shrink-0 shadow-lg">
              <span className="text-xl font-bold text-white">AAU</span>
            </div>
            {!isCollapsed && (
              <motion.div 
                initial={{ opacity: 0 }}
              ...
              >
                <div className="text-sm font-bold leading-tight text-white">AMMAN ARAB</div>
                <div className="text-[10px] text-accent uppercase tracking-widest">UNIVERSITY</div>
              </motion.div>
            )}
          </Link>
        </div>
              ...
              return (
                <li key={item.key}>
                  <Link
                    ...
                    `}
                  >
                    <Icon size={22} className={`flex-shrink-0 ${isActive ? 'text-accent' : 'group-hover:text-accent'}`} />
                    {!isCollapsed && (
                      <motion.span 
                    ...
                    {isCollapsed && (
                      <div className={`absolute ${isRtl ? 'right-full mr-2' : 'left-full ml-2'} px-2 py-1 bg-primary text-secondary text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-[70] border border-accent/20`}>
                        {t[item.key as keyof typeof t]}
                      </div>
                    )}
                  </Link>
        ...
        <div className="absolute inset-0 pointer-events-none opacity-[0.05] overflow-hidden">
          <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="islamic-sidebar" x="0" y="0" width="160" height="160" patternUnits="userSpaceOnUse">
                <path d="M80 20 Q80 10, 85 10 L85 30 Q85 40, 80 50 Q75 40, 75 30 L75 10 Q80 10, 80 20" fill="none" stroke="currentColor" strokeWidth="0.5" />
                <path d="M40 100 L40 130 Q40 140, 50 140 L70 140 Q80 140, 80 130 L80 100" fill="none" stroke="currentColor" strokeWidth="0.5" />
                <circle cx="120" cy="60" r="10" fill="none" stroke="currentColor" strokeWidth="0.5" />
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#islamic-sidebar)" className="text-accent" />
          </svg>
        </div>
      </motion.aside>
    </>
