...
export default function CallForPapers({ language }: CallForPapersProps) {
  ...
  const rules = [
    {
      en: "Paper must align with conference themes",
      ar: "يجب أن تتماشى الورقة مع محاور المؤتمر",
    },
    {
      en: "Subject to double-blind peer review",
      ar: "تخضع الورقة لمراجعة الأقران مزدوجة التعمية",
    },
    {
      en: "Accepted languages: Arabic & English",
      ar: "اللغات المقبولة: العربية والإنجليزية",
    },
    {
      en: "Original, unpublished work only",
      ar: "العمل أصيل وغير منشور مسبقاً فقط",
    },
    {
      en: "Maximum length: 20 pages or 10,000 words",
      ar: "الحد الأقصى للطول: 20 صفحة أو 10,000 كلمة",
    },
    {
      en: "Citation style: APA 7th edition",
      ar: "نمط الاقتباس: الإصدار السابع من APA",
    },
  ];

  const formatting = [
    {
      en: "Format: Microsoft Word (.docx)",
      ar: "التنسيق: Microsoft Word (.docx)",
    },
    { en: "Font: Times New Roman", ar: "الخط: Times New Roman" },
    {
      en: "Specific font sizes and spacing apply",
      ar: "تطبق أحجام خطوط وتباعد محددة",
    },
  ];
          ...
          <div className="bg-primary rounded-[32px] p-8 md:p-12 text-white flex flex-col md:flex-row items-center gap-8 shadow-2xl shadow-primary/20 border border-accent/10 relative overflow-hidden">
            ...
            <div className="flex-grow text-center md:text-start relative z-10">
              ...
              <p className="text-secondary/70 mb-4">
                {isRtl
                  ? "ترسل الأوراق البحثية عبر البريد الإلكتروني المخصص:"
                  : "Submit your papers via the dedicated conference email:"}
              </p>
          ...
          <section>
            ...
            <div className="space-y-4">
              {rules.map((rule, i) => (
                <div
                  key={i}
                  className="flex items-start gap-4 p-5 bg-white rounded-2xl border border-neutral-100 shadow-sm hover:border-accent/30 transition-colors"
                >
          ...
          <section>
            ...
            <div className="space-y-4 mb-10">
              {formatting.map((item, i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 p-5 bg-white rounded-2xl border border-neutral-100 shadow-sm"
                >
            ...
            <a
              href="/CISRAI-Temp.docx"
              download
              className="mb-10 flex items-center justify-center gap-3 px-6 py-4 bg-primary text-white rounded-2xl hover:bg-accent transition-all shadow-lg hover:shadow-xl hover:scale-105 border border-accent/20"
            >
              <Download size={22} className="text-accent" />
              <span className="font-bold text-lg">
                {isRtl
                  ? "تحميل قالب الورقة البحثية"
