import { MapPin, Navigation, Phone, Clock, Car, Plane, Mail, ShieldCheck } from 'lucide-react';
...
export default function Venue({ language }: VenueProps) {
  ...
  const t = {
    en: {
      ...
      aboutText: 'Amman Arab University (AAU) is a leading institution in Jordan, committed to academic excellence and scientific research. The campus provides a modern and inspiring environment for scholars to share knowledge and foster innovation in Islamic sciences.',
    ...
    ar: {
      ...
      aboutText: 'تعد جامعة عمان العربية مؤسسة رائدة في الأردن، ملتزمة بالتميز الأكاديمي والبحث العلمي. يوفر الحرم الجامعي بيئة حديثة وملهمة للعلماء لتبادل المعرفة وتعزيز الابتكار في العلوم الشرعية.',
        ...
        <div className="relative mb-20 overflow-hidden rounded-[40px] shadow-2xl shadow-primary/10 border border-neutral-100">
          <div className="bg-primary aspect-[21/9] w-full flex items-center justify-center relative overflow-hidden">
            ...
            <div className="absolute inset-0 opacity-10">
              <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <pattern id="venue-pattern" x="0" y="0" width="80" height="80" patternUnits="userSpaceOnUse">
                    <path d="M40 0 L50 30 L80 40 L50 50 L40 80 L30 50 L0 40 L30 30 Z" fill="white" />
                  </pattern>
                </defs>
            ...
            >
              <div className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-md px-6 py-2 rounded-full border border-white/20 text-secondary mb-6 shadow-lg">
                ...
                <span className="text-sm font-bold uppercase tracking-widest">{isRtl ? 'الموقع' : 'Location'}</span>
              </div>
              <h2 className="text-4xl md:text-6xl font-black text-white mb-4 leading-tight">{t.venueName}</h2>
              <p className="text-xl text-secondary/80 font-medium">{t.address}</p>
            </motion.div>
          </div>
          ...
          <div className="bg-white p-8 md:p-12 flex flex-col md:flex-row justify-between items-center gap-8 border-t border-neutral-100">
            <div className="flex flex-wrap gap-8 items-center justify-center md:justify-start">
              <div>
                <div className="text-primary/40 text-xs font-bold uppercase tracking-widest mb-1">{isRtl ? 'الهاتف' : 'Phone'}</div>
                <div className="font-bold text-primary">+962 6 4790222</div>
              </div>
              <div className="w-px h-10 bg-neutral-100 hidden md:block" />
              <div>
                <div className="text-primary/40 text-xs font-bold uppercase tracking-widest mb-1">{isRtl ? 'البريد' : 'Email'}</div>
            ...
            <button className="w-full md:w-auto px-8 py-4 bg-primary text-white rounded-2xl font-bold flex items-center justify-center gap-3 transition-all shadow-xl shadow-primary/20 hover:bg-accent group active:scale-95">
              ...
              <span>{isRtl ? 'عرض الموقع على الخريطة' : 'View on Google Maps'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <section className="space-y-8">
            <div className="bg-white p-8 md:p-12 rounded-[40px] border border-neutral-100 shadow-xl shadow-primary/5 relative overflow-hidden">
              <h2 className="text-3xl font-bold text-primary mb-6">{t.aboutVenue}</h2>
          ...
          <section className="space-y-6">
            <h2 className="text-3xl font-bold text-primary mb-6">{t.gettingThere}</h2>
            <div className="grid grid-cols-1 gap-4">
              <div className="bg-white p-8 rounded-[32px] border border-neutral-100 flex gap-6 items-start hover:border-accent/20 transition-all shadow-sm">
                ...
                <div>
                  <h3 className="font-bold text-lg text-primary mb-2">{isRtl ? 'عبر الجو' : 'By Air'}</h3>
              ...
              <div className="bg-white p-8 rounded-[32px] border border-neutral-100 flex gap-6 items-start hover:border-accent/20 transition-all shadow-sm">
                ...
                <div>
                  <h3 className="font-bold text-lg text-primary mb-2">{isRtl ? 'عبر الطريق' : 'By Road'}</h3>
                  <p className="text-neutral-500 text-sm leading-relaxed">
                    {isRtl 
              ...
              <div className="bg-primary p-8 rounded-[32px] text-white flex gap-6 items-center shadow-lg border border-accent/10">
                ...
                <div>
                  <h3 className="font-bold text-lg mb-1 text-white">{isRtl ? 'ساعات العمل' : 'Opening Hours'}</h3>
                  <p className="text-secondary/60 text-sm">
                    {isRtl ? 'من الأحد إلى الخميس: 08:00 - 16:00' : 'Sun - Thu: 08:00 AM - 04:00 PM'}
                  </p>
                </div>
              </div>
            </div>
